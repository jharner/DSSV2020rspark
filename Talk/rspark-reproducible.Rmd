...
The version tag of the Docker Hub images should match the version tag of the GitHub project at any given step in the development process for the images and the project repo. 
...
* Build the computing environment first, at least approximately. 
  ...
  + write a Dockerfile extending the base image, e.g., R packages  
...
By default, the containers within a Docker application are on the same network. Communication occurs through defined ports.

What if we want additional services that you only want to run occasionally, e.g., Hadoop or Spark? Or, what if we want to run some services locally and some services on a cloud platform?
...
* `rcompute` with containers `rconnect` and `rpsql` 
* `rhadoop` with containers `rhive` and `rhadoop` 
* `rspark-cluster` with conainers `master` and two workers 
...
## Anatomy of a Docker Application
Docker Container Objects:
+ Image
+ [Volumes](https://docs.docker.com/storage/volumes/)
    - [`tmpfs` Mounts](https://docs.docker.com/storage/tmpfs/) This is useful to temporarily store sensitive files that you don’t want to persist in either the host or the container writable layer.
    - [Storage Drivers](https://docs.docker.com/storage/storagedriver/select-storage-driver/) [About Storage Drivers](https://docs.docker.com/storage/storagedriver/)
    - [Bind Mounts](https://docs.docker.com/storage/bind-mounts/)

+ Network
+ Container
[Docker Documentation](https://docs.docker.com/get-started/overview/#docker-architecture)
<!-- Image -> (initializes to) Container (with volumes, networks, mounts, and plugins) -->
...
+ Services:  One or more containers executing programs.  These containers may be derived from one or more images.

+ Volumes:  The filesystems of each image can be selectively mounted as volumes to store data generated from the application.

+ Networks:  One or more networks may be established to manage service connections between containers and the host system.

+ [Volumes](https://docs.docker.com/storage/volumes/)
    - [`tmpfs` Mounts](https://docs.docker.com/storage/tmpfs/) Temporary mounts that do not store data within the container or on the host.
    - [Storage Drivers](https://docs.docker.com/storage/storagedriver/select-storage-driver/) [About Storage Drivers](https://docs.docker.com/storage/storagedriver/)
    - [Bind Mounts](https://docs.docker.com/storage/bind-mounts/)
...
+ A set of instructions outlining the construction of the container.
+ Instructions in the Dockerfile create the layers constituting the resulting image.  If the build process proceeds to completion, the resulting image can be run utilizing any executable files on its filesystem.

## Services:

Containers coupled with execution specifications.  May also include:
...
Each service completes *tasks* based on instructions from an *orchestrator*.



## Volumes:

+ Data storage mechanism for containers.
+ Preferable to using the 'container layer', which is the active, writable layer of the running container image.
+ Storage drivers manage data flow between image layers and between images and volumes.
...
+ Docker has several built-in drivers:
    - The `bridge` driver for local networks, which is the default on Docker Desktop.
    - The `overlay` driver for distributed networks, which works with `docker swarm`.
    - Custom driver specifications in the `docker-compose.yml` file can control networking configurations.

##

## Alternative Frontends

The "edge" application, in particular `rconnect`, which contains RStudio Server, can be replaced by other interfaces to Spark. Initial work is being done on `rvim`, which is a Linux container with R installed along with `vim`. We are exploring the `Nvim-R` plugin for `vim`. This is for programmers who prefer a CLI to automate all aspects of the project development.

[Rc$^2$](https://github.com/rc2server) is a native Swift client that communicates with a Swift cloud-based appserver, which in turn can spawns `computes` instances using Kubernetes. The compute instances in turn can communicate with `rhadoop` and `rspark-cluster` to get Hadoop and Spark services.
...
digraph boxes_and_circles {
    ...
    Images; Compose; Services

    node [shape=circle, fixedsize=true, width=1.0]
    Volumes; Networks; Plugins

    node [shape=circle, fixed_size=true, width=0.5]
    volume_1; volume_2; volume...; volume_n;
    network_1; network_2; network...; network_n;
    plugin_1; plugin_2; plugin...; plugin_n
    # Edges
    Images->Compose Compose->Services Volumes->Compose Networks->Compose Plugins->Compose
    volume_1->Volumes volume_2->Volumes volume...->Volumes volume_n->Volumes
    network_1->Networks network_2->Networks network...->Networks network_n->Networks
    plugin_1->Plugins plugin_2->Plugins plugin...->Plugins plugin_n->Plugins
