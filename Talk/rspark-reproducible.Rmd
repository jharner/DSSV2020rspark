...
Answer: build a Docker application with `docker-compose`, which builds multiple images and runs the corresponding containers. 

`rcompute` is Docker applications with two containers: one called `rconnect` with an RStudio service the other called `rpsql` with a PostgreSQL service. 
...
`rcompute` is an "edge" application and typically runs on your laptop or on a server. This application will probably be adequate for most of your development work, particularly since it can also run a single-node version of Spark for machine learning development.
