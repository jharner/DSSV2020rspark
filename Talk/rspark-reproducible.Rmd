...
At a minimum, a referee for a journal paper should be able to reproduce the author's work, i.e., same data, same code, but different user/machine.

Replicability is used for competitions and for studies in which the same data, but nothing else, are given to multiple research teams.

Generalizability is used, e.g., productizing machine learning algorithms, in which new data will be arriving.
...
## Steps for Creating a Reproducible Workflow
...
* Build the computing environment first, at least approximately. 
  ...
  + write shell scripts to automate building and running containers and pushing images to Docker Hub, etc.

Your container (a running instance of an image) must have a development environment, e.g., RStudio, for running code using literate programming---R Markdown is the preferred framework. 

* Build the coding environment. 
  + start a project repository on GitHub, initially with a README file  
  + clone the GitHub repo to the container runtime 
...
Answer: build a Docker application with `docker-compose`, which builds the images and runs the containers. 

`rcompute` is Docker applications with two container: one called `rconnect` with an RStudio service the other called `rpsql` with a PostgreSQL service. 

See: [rcompute](https://github.com/jharner/rcompute)
...
`rcompute` is an edge application and typically would be run on your laptop or on a server. This application will probably do for most of your development work, particularly since it can also run a single-node version of Spark for machine learning development.

`rhadoop` and `rspark-cluster` will run more efficiently in the cloud, e.g., in AWS, Digital Ocean, etc., but they can be run locally or on a server (at least for development).
