...
* **Strongly Reproducilble**: same data, but different code and users $\leadsto$ comparable results \   
...
Ex.: Productionizing machine learning algorithm
...
Boths volumes and bind mounts allow you share files between the host machine and container so that you can persist data even after the container is stopped.
...
* Docker has several built-in drivers:
    ...
    + The [**overlay** driver](https://docs.docker.com/network/overlay/) for distributed networks, i.e., containers running on different hosts, e.g., for a **docker swarm**.
    + Custom driver specifications in the **docker-compose.yml** file can control networking configurations.
    
In `rconnect` PostgreSQL can be reached by: \
postgres://rpsql:5432

Likewise, Spark can be reached by: \
spark://master:7077

See: [https://github.com/jharner/rspark-tutorial](https://github.com/jharner/rspark-tutorial)
...
See the Reports directory in the repo for this talk: [https://github.com/jharner/DSSV2020rspark](https://github.com/jharner/DSSV2020rspark)
...
Nice speedup, but not substantial since `sparklyr` already collects data in columnar format.

* Custom transformations using R functions in Spark

`spark_apply` with an R function converts the row format of Spark DataFrames to columnar format in parallel. No serializaton or deserializatoin needed. The speedup is substantial.
...
The Docker application would simply pull the required versioned `rspark` images from Docker Hub using a `docker-compose` file, i.e., the user would not need to write a Dockerfile. Further the versioned paper repo would be cloned into the `rconnect` image, allowing the editor and referees to reproduce the paper simply by calling `make`. Further, they could experiment with the code, edit the text, and even install additional R packages. 
